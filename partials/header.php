...
// Shared helpers are already loaded by bootstrap.php — no need to require again
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse ms-md-3" id="topnav">
      <ul class="navbar-nav ms-auto align-items-center gap-2">
        ...
        <?php
          $hcCandidates = [
            'health_check/index.php'                         => 'Upload & Import',
            'health_check/health_check.php'                  => 'Upload & Import',
            'health_check/health_check_inline_details.php'   => 'Inline: ESHOP → MySQL',
            'health_check/health_check_lindy_details.php'    => 'LINDY: ESHOP → MySQL',
            'health_check/health_check_wentronic_details.php'=> 'Wentronic: ESHOP → MySQL',
            'health_check/health_check_efb_details.php'      => 'EFB: ESHOP → MySQL',
            'health_check/health_check_roline_details.php'   => 'Roline: ESHOP → MySQL',
          ];
          $hcFilesExisting = [];
          foreach ($hcCandidates as $file => $label) if (exists_rel($file)) $hcFilesExisting[$file] = $label;
          $hcActive = active_any_of(array_keys($hcFilesExisting));
        ?>
        <?php if ($hcFilesExisting): ?>
          <li class="nav-item dropdown">
            <button class="btn btn-sm btn-outline-info dropdown-toggle<?= $hcActive ?>" data-bs-toggle="dropdown">Health Check</button>
            <ul class="dropdown-menu dropdown-menu-dark dropdown-menu-end">
              <?php foreach ($hcFilesExisting as $file => $label): ?>
        ...
        <?php
          $paramCandidates = [
            'health_check/health_parameters_mapper.php'            => 'Param groups mapper',
            'health_check/health_check_parameters_by_category.php' => 'Parameters by category',
            'health_check/health_check_validate_parameters.php'    => 'Validate parameters',
          ];
          $paramFilesExisting = [];
