...
/**
 * Inline Health — MSSQL (Zkratka20='eshop', Kod LIKE 'In%') missing in MySQL hc_inline
 * Location: /health_check/inline_health.php
 */
...
$INLINE_TABLE     = hc_table_name('inline');              // -> hc_inline
$INLINE_PREFIX    = (string)hc_cfg('inline_prefix', 'In');
...
$sent   = new debug_sentinel('hc_inline_health', $pdoMy);

$limit  = max(100, (int)($_GET['limit'] ?? 50000));
$export = (int)($_GET['export'] ?? 0);

/** 1) Build set of Inline article numbers present in MySQL */
$myRows = qall("
...
foreach ($myRows as $r) {
    ...
    if ($k !== '') $inlineSet[$k] = true;
}
$inlineCount = count($inlineSet);
...
$stMs = qexec(
    $sqlMs,
    [$GROUP_ID_FILTER, $INLINE_PREFIX . '%', mb_strtolower($FILTER_ZKRATKA20)],
    null,
    $MSSQL_SERVER_KEY
);
...
while ($row = $stMs->fetch()) {
    ...
    $prefixLen = mb_strlen($INLINE_PREFIX);
    if (mb_strtolower(mb_substr($kod, 0, $prefixLen)) !== mb_strtolower($INLINE_PREFIX)) continue;
    $article = trim(mb_substr($kod, $prefixLen));
    if ($article === '') continue;

    if (!isset($inlineSet[mb_strtolower($article)])) {
        $missing[] = [
            'kod'       => $kod,
...
/** Log summary */
$sent->record('inline_health_summary_eshop_only', [
    'inline_table'     => $INLINE_TABLE,
    'inline_in_mysql'  => $inlineCount,
...
if ($export) {
    ...
    header('Content-Disposition: attachment; filename=inline_missing_eshop_in_mysql.csv');
...
<div class="container my-4">
  <h1 class="mb-3">
    Inline Health — MSSQL (Zkratka20 = “<?= htmlspecialchars($FILTER_ZKRATKA20) ?>”) missing in MySQL
    <small class="text-secondary d-block fs-6">
      Filters: Kod LIKE '<?= htmlspecialchars($INLINE_PREFIX) ?>%', Group_ID = <?= htmlspecialchars($GROUP_ID_FILTER) ?>
    </small>
  </h1>
  ...
  <div class="card mb-3">
    <div class="card-body">
      <div class="row g-3">
        <div class="col-md-3">
          <div class="small text-secondary">Inline MySQL table</div>
          <div class="fw-bold"><?= htmlspecialchars($INLINE_TABLE) ?></div>
        </div>
        <div class="col-md-3">
          <div class="small text-secondary">Inline articles in MySQL (COALESCE(article_number, code))</div>
          <div class="fw-bold"><?= number_format($inlineCount) ?></div>
        </div>
        <div class="col-md-3">
          <div class="small text-secondary">
            MSSQL scanned (Kod LIKE '<?= htmlspecialchars($INLINE_PREFIX) ?>%', Zkratka20='<?= htmlspecialchars($FILTER_ZKRATKA20) ?>')
          </div>
  ...
  <div class="card">
    <div class="card-header fw-bold">
      MSSQL products with Zkratka20 = “<?= htmlspecialchars($FILTER_ZKRATKA20) ?>” missing in Inline MySQL
    </div>
