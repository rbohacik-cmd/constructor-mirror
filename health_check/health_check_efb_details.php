...
/**
 * EFB Health — EXACT match: MSSQL.Kod vs MySQL hc_efb.code
 * Location: /health_check/efb_health.php
 */

require_once __DIR__ . '/bootstrap.php'; // brings db(), db_for(), hc_* helpers, sentinel
...
$EFB_TABLE        = hc_table_name('efb'); // -> 'hc_efb'
$FILTER_ZKRATKA20 = (string)hc_cfg('filter_zkratka20', 'eshop');
$FILTER_ZKRATKA12 = (string)hc_cfg('filter_zkratka12', 'EFB');
$GROUP_ID_FILTER  = (string)hc_cfg('group_id_filter',  '139C441A-DAF2-4717-9A10-7CA8C2BFAA2E');

$pdoMy  = db();
$pdoMs  = db_for($MSSQL_SERVER_KEY);
$sent   = new debug_sentinel('hc_efb_health', $pdoMy);

$limit  = max(100, (int)($_GET['limit'] ?? 50000));
$export = (int)($_GET['export'] ?? 0);

/** 1) Exact-match set from MySQL: ONLY code (no TRIM/lower) */
$myRows = qall("
    SELECT DISTINCT code AS c
    FROM `{$EFB_TABLE}`
    WHERE code IS NOT NULL AND code <> ''
");
$efbSet = [];
foreach ($myRows as $r) {
    $k = (string)$r['c']; // keep as-is for EXACT match
    if ($k !== '') $efbSet[$k] = true;
}
$efbCount = count($efbSet);
...
while ($row = $stMs->fetch()) {
    ...
    // EXACT: MSSQL.Kod must exist as hc_efb.code
    if (!isset($efbSet[$kod])) {
        $missing[] = [
            'kod'            => $kod,
            'expected_field' => 'code',
...
$sent->record('efb_health_exact_match', [
    'efb_table'          => $EFB_TABLE,
    'efb_in_mysql'       => $efbCount,
    'mssql_scanned'      => $msCount,
    'missing_count'      => $missingCount,
    'match_mode'         => 'EXACT: MSSQL.Kod === MySQL.code',
...
if ($export) {
    ...
    header('Content-Disposition: attachment; filename=efb_missing_exact_EFB_eshop.csv');
...
<div class="container my-4">
  <h1 class="mb-3">
    EFB Health — EXACT match: MSSQL <code>Kod</code> vs MySQL <code><?= htmlspecialchars($EFB_TABLE) ?>.code</code>
    <small class="text-secondary d-block fs-6">
      Filters: Zkratka20='<?= htmlspecialchars($FILTER_ZKRATKA20) ?>', Zkratka12='<?= htmlspecialchars($FILTER_ZKRATKA12) ?>', Group_ID=<?= htmlspecialchars($GROUP_ID_FILTER) ?> (no prefix; case-sensitive compare in PHP)
    </small>
  </h1>
  ...
  <div class="card mb-3">
    <div class="card-body">
      <div class="row g-3">
        <div class="col-md-3">
          <div class="small text-secondary">EFB MySQL table</div>
          <div class="fw-bold"><?= htmlspecialchars($EFB_TABLE) ?></div>
        </div>
        <div class="col-md-3">
          <div class="small text-secondary">Distinct codes in MySQL</div>
          <div class="fw-bold"><?= number_format($efbCount) ?></div>
        </div>
        <div class="col-md-3">
  ...
  <div class="card">
    <div class="card-header fw-bold">MSSQL products missing in MySQL (exact Kod → code)</div>
