...
require_once $projectDir . '/appcfg.php';
require_once $projectDir . '/db.php';
...
/**
 * Try multiple search roots for a relative file path (e.g. '/ftp_runs.php')
...
// ---------------- FILES ----------------
// Define *logical* files and where to look for each group.
$searchRoots = [
  'ui'     => ['',],                       // root
  'logic'  => ['',],                       // root
  'js'     => ['/assets/js', '/js'],       // primary + fallback
  'lib'    => ['',],                       // root/lib already included in rel paths
  'assets' => ['/assets',],                // /assets/...
];

// Put your expected relative paths here (relative to group roots)
$need = [
  'ui' => [
    '/ftp_connections.php',
...
foreach ($need as $group => $files) {
  ...
  foreach ($files as $rel) {
    ...
    add_result('FILES', $group, 'OK', $rel . " â€“ " . $where);
  }
}
...
if ($wantJson) {
  ...
  echo json_encode(['status'=>$hadFail?'fail':'ok','results'=>$results],JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
  exit;
}

include $projectDir . '/partials/header.php';
?>
...
<?php
include $projectDir . '/partials/footer.php';
if ($hadFail && !$IS_CLI) { http_response_code(500); }
