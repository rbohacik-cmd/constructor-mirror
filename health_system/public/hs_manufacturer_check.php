...
/* --- Sorting config (kept same as original) --- */
...
/* --- Other filters from your original tool --- */
$FILTER_ZKRATKA20  = (string)hs_cfg('filter_zkratka20', 'eshop');
$GROUP_ID_FILTER   = (string)hs_cfg('group_id_filter',  '139C441A-DAF2-4717-9A10-7CA8C2BFAA2E');
$eshopOnly         = (int)($_GET['eshop_only'] ?? 0) === 1;

/* --- Sortable keys mapping (kept) --- */
$sortable = [
  'eol' => [
    'kod'        => ['kod',        'alpha'], // we store MSSQL code (Kod/Katalog) under 'kod'
  ...
  'badean' => [
    'code'       => ['Kod',       'alpha'], // we store MSSQL code (Kod/Katalog) under 'Kod'
    'nazev'      => ['Nazev',     'alpha'], // MSSQL
    'ean'        => ['EAN',       'alpha'], // HS
    'carovykod'  => ['CarovyKod', 'alpha'], // MSSQL
    'stock'      => ['stock',     'num'],
    'eta'        => ['eta',       'date'],
  ],
];
...
/* ---------------- Small helpers (kept same as your original) ---------------- */
function hs_build_url(array $overrides = []): string {
  $q = array_merge($_GET, $overrides);
  return '?' . http_build_query($q);
}

/** Keep only digits for EAN/Čiarový kód comparison. */
...
function hs_zkratka_matches(?string $val, string $prefix): bool {
  if ($val === null || $val === '') return false;
  $v = mb_strtolower(trim($val));
  $p = mb_strtolower(trim($prefix));
  return mb_substr($v, 0, mb_strlen($p)) === $p;
}
...
if ($activeTab === 'eol') {
  ...
  if ($codes) {
    ...
    for ($i = 0; $i < count($codes); $i += $chunkSize) {
      ...
      $params = array_merge([$GROUP_ID_FILTER], $chunk);
    ...
    foreach ($hsRows2 as $r) {
      ...
      $zk = (string)$msMap[$code]['Zkratka20'];
      if ($eshopOnly && !hs_zkratka_matches($zk, $FILTER_ZKRATKA20)) continue;
...
<div class="container my-4">
  <h1 class="mb-3">
    ...
    <small class="text-secondary d-block fs-6">
      EOL: Vyrobce_Nazev='<?= htmlspecialchars($MS_NAME) ?>', Zkratka20='<?= htmlspecialchars($FILTER_ZKRATKA20) ?>'<?= $eshopOnly ? ' (LIKE %)' : '' ?>, Group_ID=<?= htmlspecialchars($GROUP_ID_FILTER) ?>. <br/>
      No Stock: <?= htmlspecialchars($HS_TABLE) ?>.stock ≤ 0, matched by <?= h($MS_LABEL) ?>; MSSQL Nazev shown. <br/>
  ...
  <div class="card">
    <div class="card-header fw-bold">
      <?php if ($activeTab==='eol'): ?>
        MSSQL (Vyrobce_Nazev='<?= htmlspecialchars($MS_NAME) ?>', Zkratka20='<?= htmlspecialchars($FILTER_ZKRATKA20) ?>'<?= $eshopOnly ? ' LIKE %' : '' ?>) missing in <?= htmlspecialchars($HS_TABLE) ?> — match on <?= h($MS_LABEL) ?>
      <?php elseif ($activeTab==='nostock'): ?>
        <?= htmlspecialchars($HS_TABLE) ?> — Stock ≤ 0 (present in MSSQL<?= $eshopOnly ? ', Zkratka20 like '.htmlspecialchars($FILTER_ZKRATKA20).'%' : '' ?>) — match on <?= h($MS_LABEL) ?>
      <?php else: ?>
