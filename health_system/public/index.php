  ...
  if (!$path || !is_file($path)) {
    ...
    if ($file === 'header.php') {
      echo "<!doctype html>\n<html lang=\"en\" data-bs-theme=\"dark\"><head><meta charset=\"utf-8\">".
           ...
           // NOTE: use $BASE so css resolves even when mounted under a sub-path
           "<link href=\"".h(($GLOBALS['BASE'] ?? ''))."/hs.css\" rel=\"stylesheet\"></head>".
           "<body class=\"bg-body\">";
      return;
    }
    if ($file === 'footer.php') {
...
(function () {
  ...
  // Feature flags (UI can switch to 'prefix' inputs)
...
<!-- HS UI modules (order matters; defer preserves order).
     Use $BASE to be resilient when mounted under a sub-path. -->
<script src="<?=h($BASE)?>/ui/apiClient.js"   defer onload="(window.__HS_LOADED__ ??= {}).apiClient=true"   onerror="console.error('apiClient.js failed')"></script>
<script src="<?=h($BASE)?>/ui/logsPanel.js"   defer onload="(window.__HS_LOADED__ ??= {}).logsPanel=true"   onerror="console.error('logsPanel.js failed')"></script>
<script src="<?=h($BASE)?>/ui/runProgress.js" defer onload="(window.__HS_LOADED__ ??= {}).runProgress=true" onerror="console.error('runProgress.js failed')"></script>
<script src="<?=h($BASE)?>/ui/jobModal.js"    defer onload="(window.__HS_LOADED__ ??= {}).jobModal=true"    onerror="console.error('jobModal.js failed')"></script>
<script src="<?=h($BASE)?>/ui/filePicker.js"  defer onload="(window.__HS_LOADED__ ??= {}).filePicker=true"  onerror="console.error('filePicker.js failed')"></script>
<script src="<?=h($BASE)?>/ui/jobsTable.js"   defer onload="(window.__HS_LOADED__ ??= {}).jobsTable=true"   onerror="console.error('jobsTable.js failed')"></script>
...
(function () {
  ...
  function boot() {
    ...
    if (typeof HS?.initJobsTable === 'function') {
      ...
      console.error('[HS] HS.initJobsTable() not found. Check script load order or paths.');
    }
  }
