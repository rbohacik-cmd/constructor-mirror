...
try {
  switch ($action) {
	...
	case 'runs.logs.clear': {
		try {
			// Accept both POST/GET (hs_read_input() then fallback)
			$in    = function_exists('hs_read_input') ? hs_read_input() : [];
			$runId = isset($in['run_id']) ? (int)$in['run_id'] : (int)($_POST['run_id'] ?? $_GET['run_id'] ?? 0);
			$jobId = isset($in['job_id']) ? (int)$in['job_id'] : (int)($_POST['job_id'] ?? $_GET['job_id'] ?? 0);
			...
			$pdo   = hs_pdo();
			$where = ($runId > 0) ? 'run_id = ?' : 'job_id = ?';
			$arg   = ($runId > 0) ? $runId      : $jobId;
			$scope = ($runId > 0) ? 'run'       : 'job';

			$stmt = $pdo->prepare("DELETE FROM hs_logs WHERE {$where}");
			$stmt->execute([$arg]);
			$deleted = $stmt->rowCount();

			// Optional: sentinel trace (does NOT write to hs_logs, so nothing reappears)
			if (function_exists('debug_sentinel')) {
				debug_sentinel('health_system', 'logs.cleared', json_encode([
					'scope'   => $scope,
					'target'  => $arg,
					'deleted' => $deleted,
			...
			hs_json(false, 'Failed to clear logs: ' . $e->getMessage(), 500);
		}
	}
