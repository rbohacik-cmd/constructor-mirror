...
final class ImportWorker
{
    ...
    private static array $runMfgTag = [];
    ...
    public static function start(PDO $pdo, array $in, \debug_sentinel $sentinel): array
    {
        ...
        // Ensure FINAL table exists and is up-to-date
    ...
    public static function work(PDO $pdo, int $runId, int $uploadId, \debug_sentinel $sentinel): void
    {
        ...
        // Ensure tag cache even if start() didn’t set it
        ...
        $csv = ($ext === 'csv' || $ext === 'txt')
            ? preg_replace('~\.(csv|txt)$~i', '.norm.csv', $inputPath)
        ...
        if ($mode === 'replace') {
            ...
            try {
                ...
                self::logWarn($pdo, (int)$job['id'], $runId, 'inserting', 'Swap encountered issue, kept new as final', ['error'=>$e->getMessage()]);
            }
    ...
    /**
     * Normalize CSV/TXT into canonical CSV (code, ean, name, stock[, eta]) using exact headers.
     * Manual CSV reader to preserve raw strings (no numeric coercion).
     */
    private static function csvToCsvNormalize(string $srcCsv, string $dstCsv, array $headersWanted, array $transforms): int
    {
        ...
        self::slog($s, 'csv.normalize.start', [
            'src'=>$srcCsv,'dst'=>$dstCsv,'delimiter'=>$delimiter,'encoding'=>$encoding,'path'=>'manual-parser'
        ]);

        // convert to UTF-8 temp if needed
        $workCsv = $srcCsv;
        $tmpConverted = null;
        if ($encoding !== 'UTF-8') {
            $raw = file_get_contents($srcCsv);
            if ($raw === false) throw new \RuntimeException('Cannot read CSV: '.$srcCsv);
            ...
            if ($utf8 === false || $utf8 === '') {
                throw new \RuntimeException("Failed to convert CSV from {$encoding} to UTF-8");
            }
            $tmpConverted = $srcCsv.'.utf8.tmp';
            if (file_put_contents($tmpConverted, $utf8) === false) {
                throw new \RuntimeException('Cannot write temp converted CSV: '.$tmpConverted);
            }
            $workCsv = $tmpConverted;
        }

        $fh = new \SplFileObject($workCsv, 'r');
        $fh->setFlags(\SplFileObject::READ_CSV | \SplFileObject::SKIP_EMPTY);
        $fh->setCsvControl($delimiter, $enclosure, $escape);

        // header
        if ($fh->eof()) throw new \RuntimeException('CSV appears empty');
        $header = $fh->fgetcsv();
        if (!is_array($header)) throw new \RuntimeException('Header read failed');
        $header = array_map(static fn($v) => is_scalar($v) ? (string)$v : '', $header);
        ...
        // resolve wanted indices in exact order
        $idx = [];
        foreach ($headersWanted as $h) {
            $h = (string)$h;
            if (!array_key_exists($h, $map)) {
                throw new \RuntimeException("Header not found in CSV: '{$h}'");
            }
            $idx[] = (int)$map[$h];
        }
        ...
        while (!$fh->eof()) {
            ...
            // pick desired cells as raw strings
            $picked = [];
            foreach ($idx as $i) {
                $v = $row[$i] ?? '';
            ...
            // transforms: code trim/prefix; never mutate digits format
            ...
            if ((microtime(true) - $lastHeartbeat) >= 1.0) {
                self::slog($s, 'csv.normalize.heartbeat', ['written'=>$count]);
                $lastHeartbeat = microtime(true);
            }
        }

        fclose($out);
        if ($tmpConverted && is_file($tmpConverted)) { @unlink($tmpConverted); }

        self::slog($s, 'csv.normalize.done', ['rows_total'=>$count,'path'=>'manual-parser']);
        return $count;
    }
                                    ...
                                    int $runId=null, int $uploadId=null, int $rowsTotal=0, int $jobId=0): void
    {
        ...
        // Diagnostics that influence LOCAL INFILE
        $localInfile = null;
        try {
            $vars = [
        ...
        // If server says LOCAL INFILE is OFF, use batched
        if ($localInfile === 0) {
            self::slog($s, 'loadcsv.local_infile.off', ['table'=>$table], 'warn', 'LOCAL_INFILE_OFF');
            self::logWarn($pdo, $jobId, $runId ?? 0, 'inserting', 'Using batched inserts (local_infile=0)', ['table'=>$table]);
        ...
        // Default to batched on Windows (can be overridden by config)
        $isWindows = stripos(PHP_OS_FAMILY ?? php_uname('s'), 'Windows') !== false;
        $forceBatchedCfg = (bool)self::cfg('force_batched_insert', false);
        $forceBatched = $forceBatchedCfg || $isWindows;

        if ($forceBatched) {
            self::slog($s, 'loadcsv.force_batched', ['table'=>$table, 'os'=>PHP_OS_FAMILY, 'cfg'=>$forceBatchedCfg], 'warn', 'FORCE_BATCHED');
            self::logWarn($pdo, $jobId, $runId ?? 0, 'inserting', 'Using batched inserts', ['os'=>PHP_OS_FAMILY, 'cfg_force'=>$forceBatchedCfg, 'table'=>$table]);
            self::loadCsvBatchedInsert($pdo, $csvPath, $table, $columns, $runId, $uploadId, $rowsTotal, $jobId);
            $pdo->exec("COMMIT");
            $pdo->exec("SET unique_checks=1");
            $pdo->exec("SET foreign_key_checks=1");
        ...
        try {
            ...
            if ($n === false || ($rowsTotal > 0 && (int)$n === 0)) {
                // try LF if CRLF didn’t load anything
    ...
    /** Batched loader (portable) — bumps progress & logs as it goes */
                                                 ...
                                                 int $runId=null, int $uploadId=null, int $rowsTotal=0, int $jobId=0): void
    {
        ...
        self::logInfo($pdo, $jobId, $runId ?? 0, 'inserting', 'Batched insert begin', [
            'table'=>$table, 'batch_size'=>$batchSize, 'rows_total'=>$rowsTotal
        ]);
        ...
        while (!$fh->eof()) {
            ...
            if ($n >= $batchSize) {
                ...
                // UI-visible progress log
                self::logInfo($pdo, $jobId, $runId ?? 0, 'inserting', 'Inserted batch', [
        ...
        if ($n > 0) {
            ...
            // Final UI-visible log
            self::logInfo($pdo, $jobId, $runId ?? 0, 'inserting', 'Inserted batch (final)', [
                'table'=>$table, 'rows_done'=>$done, 'rows_total'=>$rowsTotal, 'duration_ms'=>$ms
            ]);
    ...
    /** Normalize transforms into prefix-only */
    ...
    {
        // Prefix message with manufacturer tag if not already prefixed
